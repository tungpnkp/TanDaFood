 ...
 * @return [type]        [description]
 */
    public function getProductsToCategory($id, $limit = null, $opt = null, $orderBy = 'desc', $type = 'id')
    {
        ...
        $query      = (new ShopProduct)->where('status', 1)->whereIn('category_id', $arrChild)
            ...
            ->orderBy($type, $orderBy)->orderBy('id', 'desc');
        ...
        if ($opt == 'hot') {
            return $query->orderBy('sold','desc')->limit($limit)->get();
        }
         else {
 ...
 */
    ...
    public static function getCategoryWithProduct()
    {
        $result = self::with(['products' => function ($q){
            $q->where('status', 1)->limit(10);
        }])->get();
        return $result;
    }
