...
//End type
    ...
    {
        $query = ShopProduct::where('status', 1)->with('category');
        if ($type) {
    ...
    public static function resultSearch($keyword, $sortBy)
    {
        $product =  self::where('status', 1)
            ...
            else if($sortBy == 'SORT_BY_DISCOUNT') {
                $product->orderBy('id', 'desc');
            }
            else if ($sortBy == 'SORT_BY_PRICE_DOWN') {
                $product->orderBy('price', 'desc');
            }
            else if ($sortBy == 'SORT_BY_PRICE_UP') {
 ...
 */
    ...
    public static function getAnotherBrandProduct ($brandId, $limit = 4, $currProductId) {
        return self::where('brand_id', $brandId)->where('id','!=', $currProductId)->limit($limit)->get();
    }
