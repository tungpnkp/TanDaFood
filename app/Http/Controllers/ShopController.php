...
//        View::share('logo', Banner::where('status', 1)->where('type', 0)->orderBy('sort', 'desc')->orderBy('id', 'desc')->first());
    ...
    {
//        $products = ShopProduct::with('category')->where('status', 1)->orderBy('id', 'desc')->paginate(12);
        $categoryWithProduct = ShopCategory::getCategoryWithProduct();
        return view(  'UI.shop_home', array(
    ...
    {
        $parts = explode('_', $category_name);
        $id = end($parts);
        $category = ShopCategory::find($id);
        $products = ShopProduct::with('category')->where('status', 1)->where('category_id', $id)->orderBy('id', 'desc')->paginate(12);

        return view(  'UI.shop_category', array(
            'products' => $products,
            'category' => $category,
        ));
    }
